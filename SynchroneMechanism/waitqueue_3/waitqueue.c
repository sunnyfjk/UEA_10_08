 ...
 * @Last modified time: 2018-10-23T10:17:49+08:00
 * @License: GPL
 */
#include "waitqueue.h"
MODULE_LICENSE("GPL");
...
ssize_t my_stack_read(struct file *file, char __user *buffer, size_t size,
  ...
  spin_lock(&s->lock);
  if ((file->f_flags & O_NONBLOCK) && (s->stack->top == 0)) {
    spin_unlock(&s->lock);
    return 0;
  }
  if (!(file->f_flags & O_NONBLOCK) && (s->stack->top == 0)) {
    DEFINE_WAIT(__wait);
    do {
      prepare_to_wait(&s->ro, &__wait, TASK_INTERRUPTIBLE);
      if (s->stack->top > 0) {
        break;
      }
...
ssize_t my_stack_write(struct file *file, const char __user *buffer,
  ...
  struct stack_cdev_t *s = file->private_data;
  spin_lock(&s->lock);
  if ((file->f_flags & O_NONBLOCK) && (s->stack->top == s->stack->stack_size)) {
    spin_unlock(&s->lock);
    return 0;
  }
  if (!(file->f_flags & O_NONBLOCK) &&
    ...
    DEFINE_WAIT(__wait);
    do {
      /*prepare_to_wait 使用了之后一定要使用 finish_wait 函数*/
      prepare_to_wait(&s->wo, &__wait, TASK_INTERRUPTIBLE);
      ...
      schedule();
      spin_lock(&s->lock);
    } while (s->stack->top >= s->stack->stack_size);
    /*finish_wait 函数之前一定要使用 prepare_to_wait 函数*/
    finish_wait(&s->wo, &__wait);
  }
